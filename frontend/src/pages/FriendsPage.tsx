...
export function FriendsPage() {
    ...
    // Track sent request IDs in component state for immediate UI feedback
    ...
    const handleSendFriendRequest = async (toPlayerId: string) => {
        ...
        try {
            ...
            // FIX/Cleanup: Clear the local sent state for the sent request upon successful mutation.
            setLocalSentRequests(prev => {
                const newSet = new Set(prev);
                newSet.delete(toPlayerId);
                return newSet;
            });
    ...
    const handleAcceptFriendRequest = async (fromPlayerId: string) => {
        ...
        try {
            ...
            const errorMsg = error?.response?.data?.message || "Failed to accept friend request";
            setSnackbar({ open: true, message: errorMsg, severity: "error" });
        }
    };
    ...
    const handleRejectFriendRequest = async (fromPlayerId: string) => {
        ...
        try {
            ...
            const errorMsg = error?.response?.data?.message || "Failed to reject friend request";
            setSnackbar({ open: true, message: errorMsg, severity: "error" });
        }
    };
    ...
    // Check if a player already has a relationship with the current user
    const getPlayerStatus = (playerToCheckId: string): 'friend' | 'pending_received' | 'pending_sent' | 'none' => {
        if (friends?.some(f => f.playerId === playerToCheckId)) return 'friend';
        if (pendingRequests?.some(r => r.playerId === playerToCheckId)) return 'pending_received';
        if (sentRequests?.some(s => s.playerId === playerToCheckId) || localSentRequests.has(playerToCheckId)) {
            return 'pending_sent';
        }
        return 'none';
    };

    // Filter search results to only show players with no existing relationship
    const filteredSearchResults = useMemo(() => {
        return searchMutation.data?.filter((player: PlayerDtoWithName) => {
            ...
            >
                <Stack spacing={4}>
                    <Box>
                        ...
                        {friends && friends.length > 0 && (
                            <Chip
                                ...
                                size="small"
                                sx={{ mt: 1, bgcolor: "rgba(157, 125, 255, 0.2)", color: "#9d7dff" }}
                            />
                        )}
                    </Box>
                    ...
                    {loadingFriends ? (
                        ...
                        <Box sx={{ textAlign: "center", py: 4, color: "#d0d0e5" }}>
                            <Typography>No friends yet. Search for players below to add friends!</Typography>
                        </Box>
                    )}
                        ...
                        <>
                            <Box>
                                ...
                                <Chip
                                    ...
                                    size="small"
                                    sx={{ mt: 1, bgcolor: "rgba(255, 193, 7, 0.2)", color: "#ffc107" }}
                                />
                            </Box>
                            <Grid container spacing={2}>
                                        ...
                                        >
                                            ...
                                            <Stack direction="row" spacing={1}>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => handleRejectFriendRequest(request.playerId)}
                    ...
                    {/* Sent Requests (Optional - show what you've sent) */}
                    {sentRequests?.length ? (
                        <>
                            <Box>
                                <Typography variant="h5" sx={{ fontWeight: 600, color: "#fff" }}>
                                ...
                                <Chip
                                    ...
                                    size="small"
                                    sx={{ mt: 1, bgcolor: "rgba(100, 149, 237, 0.2)", color: "#6495ed" }}
                                />
                            </Box>
                            <Grid container spacing={2}>
                                {sentRequests.map((sent: PlayerDtoWithName) => (
                                    <Grid size={{ xs: 12, sm: 6, md: 4}} key={sent.playerId}>
                                        <Stack
                                            direction="row"
                                            alignItems="center"
                                            justifyContent="space-between"
                                        ...
                                        >
                                            ...
                                            <Chip
                                                ...
                                                label="Pending"
                                                size="small"
                                                sx={{ bgcolor: "rgba(100, 149, 237, 0.2)", color: "#6495ed" }}
                                            />
                                        </Stack>
                                    </Grid>
                                ))}
                    ...
                    <Box>
                        ...
                        {submittedQuery && !searchMutation.isLoading && filteredSearchResults.length === 0 && (
                            <Box sx={{ textAlign: "center", py: 3, color: "#d0d0e5" }}>
                                <Typography>
                        ...
                        <Grid container spacing={2}>
                            {filteredSearchResults.map((player: PlayerDtoWithName) => (
